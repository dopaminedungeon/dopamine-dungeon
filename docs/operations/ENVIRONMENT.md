...
AUTH_EMAIL_EXTENDED_RATE_LIMITS_ENABLED=true
AUTH_EMAIL_VERIFICATION_MIN_INTERVAL_SECONDS=60
AUTH_EMAIL_VERIFICATION_HOURLY_LIMIT=3
AUTH_EMAIL_VERIFICATION_DAILY_LIMIT=5
AUTH_EMAIL_RECOVERY_MIN_INTERVAL_SECONDS=60
AUTH_EMAIL_RECOVERY_HOURLY_LIMIT=3
AUTH_EMAIL_RECOVERY_DAILY_LIMIT=5
AUTH_EMAIL_RECOVERY_IP_HOURLY_LIMIT=20
AUTH_EMAIL_RECOVERY_IP_DAILY_LIMIT=50
AUTH_EMAIL_RATE_LIMIT_TTL_HOURS=48
BREVO_API_KEY=<server-only-transactional-api-key>
...
recovery endpoint to create separately domain-separated HMAC fingerprints for
normalized email addresses and canonical source IP addresses. Use a high-entropy
value unique to the environment. There is intentionally no plain SHA-256 or
hard-coded fallback because email addresses and IP addresses are guessable.
Rotating the secret invalidates active fingerprint keys but does not affect
Firebase users, reset actions, or DD application data.
...
The `AUTH_EMAIL_*` rate-limit values above are the default #332 policy. They
