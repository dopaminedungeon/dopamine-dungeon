...
| `tenants/{tenantId}` | Active; browser client SDK | `TenantContext` and tenant repository create/read; bootstrap service writes | Workspace record. Tenant isolation boundary. | Neon `workspaces` are read through `/api/me`; no canonical create API. | **Replace**. Authenticated API must atomically create workspace and owner membership. |
